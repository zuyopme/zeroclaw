---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ZeroClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ZeroClaw cron scheduling for periodic checks, or version status checks on a machine running ZeroClaw (laptop, workstation, Pi, VPS).
---
# ZeroClaw Host Hardening

## Overview

Assess and harden the host running ZeroClaw, then align it to a user-defined risk tolerance without breaking access. Use ZeroClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ZeroClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. ZeroClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ZeroClaw can access sensitive data.
...
### 2) Run ZeroClaw security audits (read-only)

As part of the default read-only checks, run `zeroclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `zeroclaw security audit` (faster, non-probing)
2. `zeroclaw security audit --json` (structured output)

Offer to apply ZeroClaw safe defaults (numbered):

1. `zeroclaw security audit --fix`

Be explicit that `--fix` only tightens ZeroClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ZeroClaw version/update status (read-only)

As part of the default read-only checks, run `zeroclaw update status`.
...
- Credential hygiene notes (location of ZeroClaw creds, prefer disk encryption)
...
- ZeroClaw security audit (re-run)
...
After ZeroClaw install or first hardening pass, run at least one baseline audit and version check:

- `zeroclaw security audit`
- `zeroclaw security audit --deep`
- `zeroclaw update status`

Ongoing monitoring is recommended. Use the ZeroClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `zeroclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `zeroclaw update status`
...
Before creating, `zeroclaw cron list` and match on exact `name`. If found, `zeroclaw cron edit <id> ...`.
If not found, `zeroclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `zeroclaw update status` (preferred for source checkouts and channels)
2. `npm view zeroclaw version` (published npm version)

## ZeroClaw command accuracy

Use only supported commands and flags:

- `zeroclaw security audit [--deep] [--fix] [--json]`
- `zeroclaw status` / `zeroclaw status --deep`
- `zeroclaw health --json`
- `zeroclaw update status`
- `zeroclaw cron add|list|runs|run`

Do not invent CLI flags or imply ZeroClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ZeroClaw compaction:
